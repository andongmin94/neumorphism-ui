...
test("serves a complete shadcn registry catalog", async () => {
  ...
  const expectedCatalog = JSON.parse(await readFile(path.join(projectRoot, "../registry/catalog.json"), "utf8"));
  const expectedItems = expectedCatalog.items.map(item => item.name);
  assert.deepEqual(
