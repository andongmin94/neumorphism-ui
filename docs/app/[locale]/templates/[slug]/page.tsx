...
export function generateStaticParams() { return locales.flatMap(locale => ["settings", "data-manager", "dashboard"].map(slug => ({ locale, slug }))); }
export async function generateMetadata({ params }: Props) {
  const { locale, slug } = await params; if (!isLocale(locale) || !["settings", "data-manager", "dashboard"].includes(slug)) return {};
  const t = templateCopy[locale];
  return createLocalizedMetadata({ locale, pathname: `/templates/${slug}`, title: slug === "dashboard" ? t.dashboard : slug === "settings" ? t.settings : t.records, description: slug === "dashboard" ? t.dashboardBody : slug === "settings" ? t.settingsBody : t.recordsBody });
}
export default async function TemplatePage({ params }: Props) {
  const { locale, slug } = await params; if (!isLocale(locale) || (slug !== "settings" && slug !== "data-manager" && slug !== "dashboard")) notFound();
  const t = templateCopy[locale]; const settings = slug === "settings"; const source = slug === "dashboard" ? analyticsSource : settings ? settingsSource : recordsSource;
...
export function Example({ records }: { records: AnalyticsRecord[] }) {
  ...
  return <div className="mx-auto w-full max-w-7xl px-5 pt-8 pb-16 sm:px-8 sm:pt-12"><article className="docs-content docs-article"><header className="docs-page-header"><Link href={localeHref(locale, "/templates")} className="text-sm">← {t.back}</Link><h1>{slug === "dashboard" ? t.dashboard : settings ? t.settings : t.records}</h1><p>{slug === "dashboard" ? t.dashboardBody : settings ? t.settingsBody : t.recordsBody}</p></header><TemplatePreview slug={slug} locale={locale} /><section className="docs-content-section mt-10" id="installation"><h2>{t.install}</h2><p>{t.base}</p><InstallCommand name={source.name} /><h3>{t.use}</h3><CopyableCode code={usage} label={t.use} multiline /><h3>{t.integration}</h3><p>{slug === "dashboard" ? t.analyticsNote : t.note}</p>{slug !== "dashboard" && <p>{t.imports}</p>}</section><section className="docs-content-section" id="installed-source"><h2>{t.source}</h2>{source.files.map(file => <details key={file.path} className="docs-disclosure"><summary><code>{file.target}</code></summary><CopyableCode code={file.content} label={file.target} multiline /></details>)}</section></article></div>;
}
